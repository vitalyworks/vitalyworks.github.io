...
function initCopyDiscord() {
  const button = document.getElementById('copyDiscordButton');
  const handle = document.getElementById('discordHandle');
  const message = document.getElementById('copyMessage');
  if (!button || !handle || !message) return;

  let resetTimer = null;
  button.addEventListener('click', async () => {
    const value = handle.textContent.trim();
    try {
      await navigator.clipboard.writeText(value);
      message.textContent = window.i18n.t('contact.copied');
    } catch {
      message.textContent = window.i18n.t('contact.copyManual', { handle: value });
    }
